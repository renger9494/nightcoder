<template>
  <div class="publish-field" :class="{ 'publish-field--is-published': isPublished }">
    <div class="publish-field__top">
      <label class="publish-field__label">
        <component
          ...
          target="_blank"
        >
          <svg-icon width="20" height="20" :color="isPublished ? '#173BEF' : '#BDBDBD'" name="open-link" />
        </component>
        <component
          :is="isPublished ? 'a' : 'span'"
      ...
      <div class="publish-field__btns">
        <button class="publish-field__btn publish-field__btn--copy" v-if="isPublished" @click="copy">
    ...
    <div class="publish-field__bottom" v-if="isPublished && !isCommunity">
      <div class="publish-field__bottom-link" @click="openLink">
        <p>Open URL</p>
        <svg-icon name="open-link-icon" width="13" height="10" color="#000000" />
      </div>
      <div class="publish-field__bottom-settings" @click="$event => openSettings($event, isHomePage)">
        ...
        <svg-icon name="setting-icon" width="12" height="12" :color="isCommunity ? '#fff' : '#000'" />
      </div>
    </div>
    <div class="publish-field__bottom" v-if="!isPublished && !isCommunity">
