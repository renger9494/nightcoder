...
export function useImageElementCommonEditorFunctionality(
  ...
  const overlayStyle = computed(() => {
    return {
      ...borderRadiusCompiledObject.value,
      background: overlayCSSObject.value.overlay,
      display: displayOverlayCSSObject.value.displayOverlay
    };
  });

  const isImgSvg = computed(() => {
    return element.value.payload.image.fullPath.match('.svg$');
  });
