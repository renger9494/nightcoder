    ...
    <div class="edit-dropdown__content">
      ...
      >
        ...
        <div class="edit-dropdown-item__content">
          <div class="edit-dropdown-item__preview-wrap" @click="changeProjectToEdit(project)">
            <picture v-if="project.previewImage">
              ...
              />
              <img class="edit-dropdown-item__preview" :src="project.previewImage.fullPath" />
            </picture>

            <img class="edit-dropdown-item__preview" v-else :src="placeholder" />
            <svg-icon v-if="isSelected(project)" name="image-border" width="120" height="70" color="#318bf5" />
          </div>
          <div class="edit-dropdown-item__toolbar">
            <a class="edit-dropdown-item__btn settings" @click="openSiteSettings(project.id)" v-tooltip="'Settings'">
              <svg-icon name="settings-icon" width="12" height="12" color="#333" />
            </a>
            <a class="edit-dropdown-item__btn" @click="duplicateProject(project.id)" v-tooltip="'Duplicate'">
              <svg-icon name="duplicate" width="12" height="12" color="#333" />
            </a>
            <a class="edit-dropdown-item__btn" @click="openDeleteProjectConfirmation(project)" v-tooltip="'Delete'">
              <svg-icon name="delete" width="12" height="14" color="#333" />
            </a>
          </div>
        </div>
      </div>
...
export default Vue.extend({
  ...
  methods: {
    ...
    openDeleteProjectConfirmation(project: BaseProjectDto) {
      const actionAfterDelete = () => {
        if (+this.$route.params.projectId == project.id) {
          location.href = `${getCabinetUrl()}/dashboard`;
        }
      };
    ...
    duplicateProject(projectId: number) {
      if (!this.isProjectsLimit) {
        this.$store.dispatch('project/duplicateProject', projectId).then(() => {
          this.$emit('showNotification', 'Project');
        });
      } else {
        this.$store.commit('ui/changeVisibleModalName', VisibleModalType.Limit);
      }
