...
export default function nestedElementCssStyleEditor({
  element,
  style,
  store,
  cssObjectOnly = true,
  ...
  const displayConditions = computed(() => store.state.editor.displayConditions[element.value.id]);

  const isCssPropertyHidden = computed(
    () => !displayConditions.value || displayConditions.value[hideCssPropertyConditionName]
  );
  ...
  const compiledCSS = computed(() => {
    if (isCssPropertyHidden.value) {
      ...
      if (cssProperty == 'color') return 'rgba(0,0,0,0)';
      return '';
    }
  ...
  const compiledCssObject = computed(() => {
    if (isCssPropertyHidden.value) {
      //because browser always sets default value rgb(1,1,1), if element doesn't have color
      if (cssProperty == 'color') return { color: 'rgba(0,0,0,0)' };
      return {};
    }
  ...
  const isCssPropertyActive = computed(() => !!cssPropertyStyle.value[cssProperty]);
