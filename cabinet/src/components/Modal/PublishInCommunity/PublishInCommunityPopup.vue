<template>
  <div class="publish-popup-wrap">
    ...
    <PublishedModal
      v-if="step == 3"
      :projectName="projectCommunity.name"
      :airtapDomainUrl="airtapDomainUrl"
    />
    <div
      :class="{ popupFooter__actions__all: step == 2 }"
      class="popupFooter__actions"
    >
      <button
        class="popupFooter__btn popupFooter__btn--transparent"
        v-if="step == 2"
      ...
      <button
        ...
        :disabled="isDisabled"
        :class="{
          loading: additionalPublishStatus == PublishStatus.InProgress,
        }"
        class="popupFooter__btn popupFooter__btn__black"
      >
        ...
        >
        <span v-else>{{
          isAlreadyProjectPublished ? 'Save Changes' : 'Publish'
        }}</span>
        <svg-icon
          :name="
            additionalPublishStatus == PublishStatus.InProgress
              ? 'loader-icon'
              : 'publish-community-icon'
          "
          width="16"
          height="16"
          color="#fff"
...
import PaidContent from '@/components/Modal/PublishInCommunity/PaidContent.vue'
import PublishSettings from '@/components/Modal/PublishInCommunity/PublishSettings.vue'
import PublishedModal from '@/components/Modal/PublishInCommunity/PublishedModal.vue'
...
export default Vue.extend({
  ...
  created() {
    ...
    this.file.path = actionButton.file?.path || ''
    this.file.name = actionButton.file?.originalFileName || ''
    this.file.size = (actionButton.file?.size || 0) * 1024 * 1024
    this.file.id = actionButton.file?.id || undefined
    this.file.file = null
    this.link = actionButton.link || ''
    this.category = null
    this.tags = undefined
  ...
  computed: {
    ...mapState({
      popupSettings: (state: any) =>
        <PopupSettings<PublishInCommunityPopupData>>state.ui.popupSettings,
    ...
    isErrorSubscribe(): boolean {
      ...
      if (!(this.user.plan.category == PlanCategory.Pro || this.user.isAdmin))
        return true
      if (
        (this.projectCommunity.actionButton.type ==
          CommunityActionButtonType.File &&
          !this.file.name) ||
        (this.projectCommunity.actionButton.type ==
          CommunityActionButtonType.Link &&
          (!this.link || this.isErrorLink))
      )
        return true
      return false
    },
    isDisabled(): boolean {
      return (
        (this.step == 1 && this.isErrorSettings) ||
        (this.step == 2 && this.isErrorSubscribe)
      )
    },
  },
  watch: {
  ...
  async mounted() {
    try {
      const { data: categories } =
        await apiClient.projectCategories.projectCategoriesGetWithTags()
      this.categories = categories
      if (this.projectCommunity.categoryId) {
        const category = this.categories.find(
          (category) => category.id === this.projectCommunity.categoryId
        )
      ...
      this.pages = pages.map((tc) => {
        ;(<ProjectPage>tc).numberPage =
          this.projectCommunity.pages?.some((id) => id == tc.id) ||
          tc.isHomePage
            ? ++numberPage
            : undefined
  ...
  methods: {
    ...
    selectedCategory(category: ProjectCategoryWithTagsDto, created: boolean) {
      ...
      this.tags = category.tags?.map((tc) => {
        const isActive = !!(
          created && this.projectCommunity.tags?.some((tag) => tag === tc.id)
        )
    ...
    selectedTag(tag: ProjectTag, index: number) {
      ...
      if (this.tags)
        Vue.set(this.tags, index, { ...tag, isActive: !tag.isActive })
    },
    ...
    changedPublished(index: number) {
      ...
      if (!page.numberPage) {
        this.pages.forEach((item) => {
          if (item.numberPage && item.numberPage > changedPage)
            item.numberPage--
        })
      }
    },
    ...
    publish() {
      ...
      http
        ...
        .then(({ data }) => {
          this.$store.commit('ui/CHANGE_POPUP_BACKGROUND', '')
          this.$store.commit('ui/CHANGE_POPUP_HEADER', '')
          this.$store.commit('ui/CHANGE_POPUP_DESCRIPTION', '')
          this.$store.commit('project/changeProjectToPublish', this.projectId)
