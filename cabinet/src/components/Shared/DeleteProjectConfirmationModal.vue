  ...
  <modal class="delete-project-confirmation-modal" :isVisible="isVisible">
    <div class="delete-project-confirmation-modal__inner">
      <img
        ...
        srcset="/img/modals/delete-project-illustration.png, /img/modals/delete-project-illustration@2x.png 2x"
        alt=""
      />
      <h2 class="delete-project-confirmation-modal__title">
        Do you really want to Delete&nbsp;<br /><span> {{ name }} </span>
        {{ modalType === DeleteModalMode.Project ? 'project' : 'page' }} ?
      </h2>
...
export default Vue.extend({
  ...
  computed: {
    ...mapState({
      deleteModalData: (state: any) =>
        <DeleteModalData>state.project.deleteModalData,
    }),
  ...
  methods: {
    remove(event: Event) {
      ...
      const { projectId, type, pageId, actionAfterDelete } =
        this.deleteModalData
      //it's only for editor
      // if (+this.$route.params.projectId == projectId) {
      //   this.$router.push('/projects')
      //   commit('editor/SET_USER_CONFIG_FROM_LOCAL', null)
      // }
