...
export enum SiteSettingsMutations {
  ...
  CHANGE_PROJECT_ACTION = 'CHANGE_PROJECT_ACTION',
}
...
const mutations = {
  ...
  [SiteSettingsMutations.CHANGE_PROJECT_PAGES](state: SiteSettingsState, projectPages: Array<any>) {
  ...
  [SiteSettingsMutations.CHANGE_ACTIVE_SECTION](state: SiteSettingsState, activeSection: number) {
...
const actions: ActionTree<SiteSettingsState, any> = {
   ...
   */
  async loadModalData({ commit }, { projectId, pageId, activeSectionNumber }: any) {
    ...
    await Promise.all([getProjectById(), getProjectPages()]).then(([{ data: project }, { data: projectPages }]) => {
      ...
      commit(
        ...
        pageId ? projectPages.find((page) => page.id == pageId) : projectPages[0]
      )
  ...
  setModalData({ commit }, { project, projectPages, pageId, activeSectionNumber }: any) {
    ...
    commit(
      SiteSettingsMutations.CHANGE_PAGE,
      pageId ? projectPages.find((page: BaseProjectPageDto) => page.id == pageId) : projectPages[0]
    )
    commit(SiteSettingsMutations.CHANGE_MODAL_MODE, pageId ? ModalMode.Page : ModalMode.Project)
    if (activeSectionNumber) {
      commit(SiteSettingsMutations.CHANGE_ACTIVE_SECTION, activeSectionNumber)
  ...
  setPage({ commit }, { page }: { page: BaseProjectPageDto }) {
